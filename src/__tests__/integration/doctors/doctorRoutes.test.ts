...
import app from "../../../app";
import { mockedDoctor, mockedDoctorLogin, mockedDoctorNotName, mockedDoctorNotPassword } from "../../mocks";
...
describe("/doctor", () => {
  ...
  test("POST /login/doctor -  should be able to login with the doctor",async () => {
    const response = await request(app).post("/login/doctor").send(mockedDoctorLogin);
    expect(response.body).toHaveProperty("id")
