...
describe("/users", () => {
  ...
  test("POST /users -  Should not be able to create a user that not exists name in body", async () => {
    const response = await request(app).post("/users").send(mockedUserNotName);
  ...
  test("POST /users -  Should not be able to create a user that CPF is invalid", async () => {
    const response = await request(app)
      .post("/users")
      .send(mockedUserCPFInvalid);

    expect(response.body).toHaveProperty("message");
    expect(response.body.message).toEqual("cpf must be at least 11 characters");
    expect(response.status).toBe(400);
  });
...
test("POST /login -  should not be able to login with the user with incorrect password or email",async () => {
  ...
  test("PATCH /users/:id - Should not be able to update user with invalid id", async () => {
    const newValues = { name: "Pedro Marcos", email: "pedro@mail.com" };
  ...
  test("PATCH /users/:id -  should be able to update user", async () => {
    const newValues = { name: "Jose", email: "jose@gmail.com" };
